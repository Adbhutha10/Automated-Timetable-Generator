...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      // Direct to the selected role's dashboard, or fallback to the one from the token
      const finalRole = formData.role || result.role;
      navigate(roleRedirects[finalRole] || from, { replace: true });
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-6 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500">Sign in to your Schedulify account</p>
        </div>
        ...
        <div className="bg-white p-10 rounded-3xl shadow-xl border border-slate-100 shadow-slate-200/50">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 ml-1 uppercase tracking-wider">Logging in as</label>
              <div className="relative">
                <select
                  required
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center ml-1">
                ...
                <Link to="/forgot-password" size="sm" className="text-xs font-bold text-primary-600 hover:text-primary-700">Forgot?</Link>
              </div>
              <div className="relative">
          ...
          <div className="mt-10 pt-8 border-t border-slate-100 text-center">
            <p className="text-slate-500 font-medium">
              ...
              <Link to="/signup" className="text-primary-600 font-bold hover:text-primary-700">Create one</Link>
            </p>
          </div>
        </div>
